...
export class ArchivalService implements OnModuleInit {
  ...
  onModuleInit() {
    ...
    setInterval(() => {
      this.runSweep().catch((err) => this.logger.error('Archival sweep failed', err));
    }, 1000 * 60 * 60);
  }

  async runSweep() {
    // Find conversations with messages array longer than MAX_MESSAGES
    const convs = await this.convModel.find({ $expr: { $gt: [{ $size: '$messages' }, MAX_MESSAGES] } }).exec();
    ...
    for (const conv of convs) {
      try {
        ...
        await this.archiveModel.create({ conversationId: conv._id, messages: messagesToArchive });
        conv.messages = conv.messages.slice(numToArchive);
        await conv.save();

        this.logger.log(`Archived ${numToArchive} messages from conversation ${conv._id}`);
      } catch (err) {
